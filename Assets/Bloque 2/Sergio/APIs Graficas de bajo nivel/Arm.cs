...
public class Arm : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        rotZ += goingUP? DX : -DX;
        matriz = Transformaciones.RotateZ(rotZ)*mOriginal;

        for(int i = 0; i < Cubos.Count; i++){
            Cubos[i].mesh.vertices = Transformaciones.Transform(multiplier(i) * Transformaciones.scale(1,0.5f,0.5f),originales);
        }
    }
